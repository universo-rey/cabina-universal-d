...
- allowed_actions: prepare local userCataloguePath mapping; review .agents/skills catalogue; propose workspace-only setting agileagentcanvas.userCataloguePath=.agents/skills; run local validators
...
- evidence: .agents/skills directory exists; .vscode/settings.json is workspace-scoped; Agile Agent Canvas governance row aac.skill_catalogue remains PENDING_TARGET_ONLY until apply order
...
This packet prepares a local workspace-scoped mapping only. It does not write
