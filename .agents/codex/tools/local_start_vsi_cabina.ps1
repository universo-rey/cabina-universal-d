...
function Start-Bridge {
  ...
  $escapedRoot = Escape-SingleQuotedPowerShell -Value $RepoRoot
  $command = "`$env:SDU_BRIDGE_PORT='$Port'; `$env:SDU_BRIDGE_BIND_HOST='127.0.0.1'; Set-Location -LiteralPath '$escapedRoot'; npm start --prefix local-agent-bridge"
  Start-Process -FilePath "powershell.exe" -ArgumentList @("-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", $command) -WorkingDirectory $RepoRoot -WindowStyle Hidden | Out-Null
}
...
if (-not (Test-BridgeHealth -BaseUrl $dashboardUrl)) {
  Start-Bridge -RepoRoot $repoRoot -Port $Port
  $bridgeStarted = $true
