...
function Test-PathToken {
  ...
  $resolved = Resolve-CabinaPath -Path $PathToken
  switch ($PathCheck) {
    "literal" {
      if (-not (Test-Path -LiteralPath $resolved)) {
        $Errors.Add("$Context missing literal path: $PathToken")
      }
    }
    "list" {
      foreach ($token in @($PathToken -split "\|" | ForEach-Object { $_.Trim() } | Where-Object { $_ })) {
        Test-PathToken -PathToken $token -PathCheck "literal" -Errors $Errors -Warnings $Warnings -Context $Context
      }
    }
    "wildcard" {
    ...
    "external_runtime" {
      if (-not (Test-Path -LiteralPath $resolved)) {
        $Warnings.Add("$Context external runtime path not present in this environment: $PathToken")
      }
    }
    default {
      $Errors.Add("$Context unsupported path_check: $PathCheck")
    }
  }
}
...
function Test-Markers {
  ...
  $resolved = Resolve-CabinaPath -Path $Row.path
...
if (-not $rootRow) {
  ...
  if ($rootRow.instruction_role -ne "local_rector_source") {
    $errors.Add("root_agents_md must be local_rector_source")
  }
}
...
foreach ($row in $rows) {
  foreach ($field in @("surface_id","path","path_check","scope","precedence_rank","instruction_role","owner_agent","reviewer_agent","allowed_actions","blocked_actions","nested_repo_policy","validator","evidence","stop_condition")) {
    if ([string]::IsNullOrWhiteSpace($row.$field)) {
      $errors.Add("Instruction surface '$($row.surface_id)' missing $field")
    }
  }
  if ($row.owner_agent -notin $agentIds) {
    $errors.Add("Instruction surface '$($row.surface_id)' references unknown owner_agent: $($row.owner_agent)")
  }
  if ($row.reviewer_agent -notin $agentIds) {
    $errors.Add("Instruction surface '$($row.surface_id)' references unknown reviewer_agent: $($row.reviewer_agent)")
  }
  if ($row.validator -notin $toolIds) {
    $errors.Add("Instruction surface '$($row.surface_id)' references unknown validator: $($row.validator)")
  }
  if ($row.validator -notin $toolGovernanceIds) {
    $errors.Add("Instruction validator lacks tool governance row: $($row.validator)")
  }
  foreach ($stop in @($row.stop_condition -split "\|" | ForEach-Object { $_.Trim() } | Where-Object { $_ })) {
    if ($stop -notin $knownStops) {
      $errors.Add("Instruction surface '$($row.surface_id)' references unknown stop_condition: $stop")
    }
  }
  $rank = 0
...
if ($nestedRow) {
  foreach ($blocked in @("absorb_nested_repo", "move_clone", "delete_branch_remote")) {
    if ($nestedRow.blocked_actions -notmatch [regex]::Escape($blocked)) {
      $errors.Add("nested_repositories row missing blocked action: $blocked")
    }
  }
}
