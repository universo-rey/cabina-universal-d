...
1. Read the queue row, owner agent, reviewer agent, dependency, lock key,
   ...
   condition.
2. If dependency is not complete, stop with the dependency-specific pending
   state and do not execute the row.
3. Confirm the write scope is exact and repo-local. If it is not exact, prepare
   an order packet instead of editing.
4. Execute the smallest local change that satisfies the row.
5. Run the row validator plus `git diff --check`.
6. Record evidence in the row output, readback, PR or validator result.
7. Move the row to `EXECUTED_LOCAL_VALIDATED` only after evidence exists;
