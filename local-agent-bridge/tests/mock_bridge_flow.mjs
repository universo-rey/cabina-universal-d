...
assert.equal(dashboard.board_boundary.status, "BOARD_BOUNDARY_DECLARED");
assert.equal(dashboard.board_boundary.live_executed, false);
assert.equal(dashboard.board_boundary.primary_board.board_id, "vsi_agile_agent_canvas_mother_board");
assert.equal(dashboard.board_boundary.primary_board.label, "Tablero principal madre VSI");
assert.equal(dashboard.board_boundary.primary_board.explicit_name, "Agile Agent Canvas");
assert.equal(dashboard.board_boundary.primary_board.board_kind, "agile_agent_canvas_creation_planning_board");
assert.equal(dashboard.board_boundary.primary_board.authority_role, "primary_mother_board");
assert.equal(dashboard.board_boundary.primary_board.surface, "vscode_insiders_agile_agent_canvas");
assert.equal(dashboard.board_boundary.primary_board.not_this_board, "control_agentes_cabina_queue_board");
assert.equal(dashboard.summary.aac_native_agent_records, 21);
assert.equal(dashboard.summary.cabina_governance_agent_records, 7);
assert.equal(dashboard.board_boundary.primary_board.native_agents.record_count, 21);
assert.equal(dashboard.board_boundary.primary_board.native_agents.status, "ACTIVE_AAC_NATIVE_TEAM_GOVERNED");
assert.equal(dashboard.board_boundary.primary_board.native_agents.team_role, "native_aac_team_not_cabina_authority");
assert.equal(dashboard.board_boundary.primary_board.native_agents.active_record_count, 21);
assert.equal(dashboard.board_boundary.primary_board.native_agents.direct_invocation_from_codex, false);
assert.equal(dashboard.board_boundary.primary_board.governance_agents.record_count, 7);
assert.equal(dashboard.board_boundary.primary_board.governance_agents.status, "ACTIVE_CABINA_GOVERNED_WORK_LAYER");
assert.equal(dashboard.board_boundary.primary_board.governance_agents.relation, "cabina_governed_work_agent_not_native_aac_team");
assert.equal(dashboard.board_boundary.primary_board.governance_agents.work_layer, "cabina_agents_work_on_board_and_govern_it");
assert.equal(dashboard.board_boundary.primary_board.agent_task_records, dashboard.agent_task_queue.length);
assert.equal(dashboard.board_boundary.primary_board.stop_condition, "vsi_mother_board_boundary_drift");
assert.equal(dashboard.board_boundary.auxiliary_board.board_id, "control_agentes_cabina");
assert.equal(dashboard.board_boundary.auxiliary_board.label, "Tablero de cola Control de Agentes de Cabina");
assert.equal(dashboard.board_boundary.auxiliary_board.explicit_name, "Control de Agentes de Cabina");
assert.equal(dashboard.board_boundary.auxiliary_board.board_kind, "agent_control_queue_board");
assert.equal(dashboard.board_boundary.auxiliary_board.authority_role, "auxiliary_queue_control_board");
assert.equal(dashboard.board_boundary.auxiliary_board.not_this_board, "vsi_agile_agent_canvas_mother_board");
assert.equal(dashboard.board_boundary.auxiliary_board.visible_gate_records, dashboard.visible_gate_lane.item_count);
assert.equal(dashboard.board_boundary.auxiliary_board.stop_condition, "agent_control_queue_board_boundary_drift");
assert.equal(dashboard.board_boundary.external_website_queue.queue_id, "cola_sitio_web");
assert.equal(dashboard.board_boundary.external_website_queue.source, "NO_CONECTADA_EN_CONTROL_AGENTES_CABINA");
assert.equal(dashboard.board_boundary.external_website_queue.records_visible_here, 0);
assert.equal(dashboard.board_boundary.external_website_queue.stop_condition, "website_queue_target_missing");
assert.equal(
...
assert.match(dashboardHtml, /Frontera de tableros/);
assert.match(dashboardHtml, /Tablero principal madre VSI/);
assert.match(dashboardHtml, /Control de Agentes de Cabina/);
assert.match(dashboardHtml, /vsi_agile_agent_canvas_mother_board/);
assert.match(dashboardHtml, /control_agentes_cabina/);
assert.match(dashboardHtml, /cola_sitio_web/);
assert.match(dashboardHtml, /NO_CONECTADA_EN_CONTROL_AGENTES_CABINA/);
assert.match(dashboardHtml, /renderBoardBoundaryItem/);
