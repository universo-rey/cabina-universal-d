param(
  ...
  [string]$CanonicalId = 'apply.tenant_controlled_gate_state_patch.20260608',
  [string]$SourcePath = '.agents/codex/readbacks/2026-06-08_tenant_controlled_gate_state_patch_readback.md',
  [string]$OutputDir = 'dataverse/validation/apply_log_publish_20260608',
  [switch]$Apply,
  [switch]$Rollback
)
...
$metadataUri = "$EnvironmentUrl/api/data/v9.2/EntityDefinitions(LogicalName='mon_sdu_apply_log')?`$select=EntitySetName,LogicalName,PrimaryIdAttribute"
$metadata = Invoke-DataverseGet -Uri $metadataUri -Headers $headers
if ($metadata.EntitySetName -ne 'mon_sdu_apply_logs') {
  throw "ENTITY_SET_MISMATCH:$($metadata.EntitySetName)"
}

$select = 'mon_canonical_id,mon_status,mon_display_name,mon_source_hash,mon_source_path,mon_stop_condition,mon_notes,mon_sdu_apply_logid'
...
$payload = @{
  ...
  mon_display_name = 'Tenant controlled gate state patch apply log 20260608'
  mon_status = 'APPLIED_POSTCHECKED'
  mon_seed_batch_id = '20260608_tenant_gate_state_patch_v1'
  ...
  mon_notes = 'Controlled Dataverse DEV metadata patch applied to two gate rows; postcheck and rollback primitive recorded in repo-local readback.'
}

$rollbackPayload = @{
  mon_status = 'ROLLBACK_SUPERSEDED'
  mon_stop_condition = 'ROLLBACK_SUPERSEDED_BY_OPERATOR_OR_PATCH_ROLLBACK'
  mon_last_reconciled_at = (Get-Date).ToString('yyyy-MM-ddTHH:mm:sszzz')
  mon_notes = 'Apply log rollback marker applied. Evidence row retained; no physical delete.'
}
...
if ($Apply) {
  if ($beforeRows.Count -eq 0) {
    ...
    $recordId = [string]$beforeRows[0].mon_sdu_apply_logid
    if ([string]::IsNullOrWhiteSpace($recordId)) { throw 'identity_field_missing:mon_sdu_apply_logid' }
...
$summary = [pscustomobject]@{
  status = if ($Apply) { 'DATAVERSE_APPLY_LOG_PUBLISH_APPLY_PASS' } elseif ($Rollback) { 'DATAVERSE_APPLY_LOG_PUBLISH_ROLLBACK_PASS' } else { 'DATAVERSE_APPLY_LOG_PUBLISH_DRY_RUN_PASS' }
  mode = $mode
  token_printed = $false
